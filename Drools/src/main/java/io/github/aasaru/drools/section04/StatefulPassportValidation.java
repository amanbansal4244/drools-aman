...
public class StatefulPassportValidation {
	...
	static void execute(int step) {
		...
		passports.forEach(ksession::insert); // we are inserting one by one all the passport into the kie session.

		System.out.println("==== DROOLS SESSION START ==== ");
		ksession.fireAllRules(); // firing all the rules
		ksession.dispose();// destroying the session, not to have memory leakage problem
